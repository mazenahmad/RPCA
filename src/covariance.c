...
/////////////////////////////////////////////////////////////////////
/********************************************************************
read and write symm. (covariance) matrix to a binary file
note: the structure of xdr defines reading and writting which 
...
int write_cov_matrix(const char *outfile,double **mat , double **average,int *ndim,int verbose)
{
	...
	if(verbose > 1)
	{
		printf("**** Testing writting the cov. matrix via rereading...\n");
...
/////////////////////////////////////////////////////////////////////////////////////
/************************************************************************************
read and write mean of state A , B ,genaralized eigen values and vectors from/to a 
binary file *.mmvv
note: the structure of xdr defines reading and writting which 
have the same function inside xdr routines. 
************************************************************************************/
...
void relative_pca_subspacing(Geigdecomposition_t *G,int ordered ,int verbose )
{
	...
	ALLO_ONE_DIM_ARRAY((G->acc_kl), double , rank );
	ALLO_ONE_DIM_ARRAY((G->kl_m), double , rank );
	double m= cblas_ddot (ndim, G->gevec, 1, dav, 1);
	for (i = 0; i < rank; i++) G->kl_m[i]=0;
	G->kl_m[0] = 0.5*m*m;
	//double lambda_m= vecX_symat_vecY( B, ndim, G->gevec  , G->gevec );
	double lambda_m= G->geigval[0] ;
	G->kl[0]= G->kl_m[0] + 0.5* (-log(lambda_m) + lambda_m -1 );
	for (i = 1; i < rank; i++)
	{
			G->kl[i] = 0.5*( -log(G->geigval[i] ) + G->geigval[i]-1 );
	}
	G->sum_kl= 0 ;
	G->sum_kl_m=G->kl_m[0];
	for (i = 0; i < rank; i++)	G->sum_kl   += G->kl[i];
	if (verbose)
	{
		printf("total DKL= %f\n",G->sum_kl );
		printf("total DKL due to variance change = %f\n",G->sum_kl - G->sum_kl_m);
	...
	if (ordered) 
	{
		...
		printf("Reordering according to the values of DKL\n" );
		printf("*********************************************\n");
		ALLO_ONE_DIM_ARRAY(kl_indx, int , rank );
		sort_decreasing( G->kl ,rank ,kl_indx);
		ALLO_ONE_DIM_ARRAY(tmp_sorted, double , rank );
		for (i = 0; i <rank; i++) tmp_sorted[i]= G->kl[kl_indx[i]];
		for (i = 0; i <rank; i++) G->kl[i]=tmp_sorted[i];
		for (i = 0; i <rank; i++) tmp_sorted[i]= G->kl_m[kl_indx[i]];
		for (i = 0; i <rank; i++) G->kl_m[i]=tmp_sorted[i];
		for (i = 0; i <rank; i++) tmp_sorted[i]= G->geigval[kl_indx[i]];
		for (i = 0; i <rank; i++) G->geigval[i]=tmp_sorted[i];
		free(tmp_sorted);
		ALLO_ONE_DIM_ARRAY(tmp_sorted, double , (ndim*rank) );
		for (i = 0; i < (ndim*rank); i++) tmp_sorted[i]=G->gevec[i] ;
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
void write_Geigen2xvg(const char *Geigfile, Geigdecomposition_t *eig )	
{		
	FILE   *out;
...
////   done by reconsturct x from y around the average of m_x in the y space 
